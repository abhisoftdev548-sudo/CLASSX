import React, { useEffect, useState } from 'react'
import useClass from '../../../classroom/hooks/useClass';
import JoinClassForm from '../../../classroom/components/JoinClassForm';
import { useNavigate } from 'react-router-dom';

const JoinedClasses = () => {
  const {getAllClasses, allJoinedClass, loading, setActiveClass} = useClass();
const navigate = useNavigate()
const [error, setError] = useState(null);
  const [activeClassForm, setActiveClassForm] = useState(false);
  async function getClasses(category){

    try {
      const enrollment = await getAllClasses({category});

    } catch (err) {
      console.error('Error fetching classes:', err);
      setError(err.message || 'Failed to fetch classes');
   
      return [];
    }
  }

  useEffect(() => {
    getClasses("joined");
  }, []);
  const classrooms = allJoinedClass
  return (
    <div className='p-5'>
      <div className='flex justify-between items-center'>

      <h2 className='text-4xl font-bold text-primary'>Joined Classes</h2>
      <button  onClick={() => setActiveClassForm(true)} 
         disabled={activeClassForm} className='text-center  px-5 py-2 rounded bg-primary text-base-100 text-lg flex items-center justify-center'><span className='text-2xl'>+</span> <span>Join a Class</span></button>
      </div>
{activeClassForm ? (
        <JoinClassForm setActiveClassForm={setActiveClassForm}/>
      ) : (
        <>
          {loading && (
            <div className='flex justify-center items-center py-10'>
              <div className='loading loading-spinner loading-lg text-primary'></div>
            </div>
          )}

          {error && (
            <div className='w-full bg-error/10 border border-error/30 p-4 rounded-lg mb-4'>
              <p className='text-error font-medium'>❌ {error}</p>
            </div>
          )}

          <div className='flex flex-wrap gap-10 mt-10'>
            {classrooms && classrooms.length > 0 ? (
              classrooms.map((classroom, index) => (
                <div 
                  key={classroom._id || index} // Use unique ID if possible
                  onClick={() => {
                    setActiveClass(classroom?.class);
                    navigate(`/class/${classroom?.enrollmentId}`);
                  }} 
                  className='bg-base-200 border border-primary/30 rounded-lg p-5 flex gap-5 cursor-pointer hover:shadow-lg transition-all items-center'
                >
                  <div>
                    <div className='w-25 h-25 bg-error text-base-100 flex items-center justify-center text-2xl rounded-full capitalize'>
                      {classroom?.class?.className?.slice(0, 2)}
                    </div>
                  </div>
                  <div>
                    <h3 className='font-bold text-secondary capitalize text-xl'>{classroom?.class?.className}</h3>
                    <p className='text-sm text-primary/80 font-bold capitalize'>{classroom?.class?.classSubject}</p>
                    <p className='text-xs text-base-content/50'>{classroom?.class?.classSession}</p>
                    <p>
                      <span className='text-sm text-secondary/80 font-medium'>Created On:</span> 
                      <span className='text-xs text-primary/60 ml-1'>
                        {classroom?.joinedAt ? new Date(classroom.joinedAt).toDateString() : "N/A"}
                      </span>
                    </p>
                       <p>
                      <span className='text-sm text-secondary/80 font-medium'>Your Role:</span> 
                      <span className='text-xs text-primary/60 ml-1 capitalize'>
                        {classroom?.role}
                      </span>
                    </p>
                  </div>
                </div>
              ))
            ) : (
              !loading && (
                <div className='w-full border-2 border-primary/50 p-10 text-center mt-20 rounded-xl'>
                  <p>Not any classes created yet</p>
                </div>
              )
            )}
          </div>
        </>
      )}

    </div>
  )
}

export default JoinedClasses
